...
| Milestone | Creates |
| --- | --- |
| M1 | scaffold, `config`, `db/` + migrations, `fs/`, `media/`, `jobs/`, `features/grid` |
| M1.5 | `fs/import.rs`, the reversal script, first-run wizard UI |
| M2 | `commands/folders`, `commands/tags`, `db/tags`, `features/folder`, `features/inspector` |
| M3 | `query/`, `commands/search`, `features/search` |
| M4 | `commands/triage`, `db/journal`, `fs/trash`, `features/triage`, `features/viewer` |
